...
"""
import os, sys, json, math, uuid, urllib.request, urllib.error
...
def test_chat():
    ...
    st, bd = _request("POST", "/chat/completions", {
...
def test_tts_asr():
    ...
    # 1) TTS 合成语音
    st, bd = _request("POST", "/audio/speech", {
    ...
    body += fdata
    body += f'\r\n--{boundary}--\r\n'.encode()
    st, bd = _request("POST", "/audio/transcriptions", None,
...
def test_image_vectors():
    ...
    def gen(prompt):
        st, bd = _request("POST", "/images/generations", {
    ...
    if not url1 or not url2:
        jprint("img-gen A", st1, str(url1)[:200]); jprint("img-gen B", st2, str(url2)[:200])
    ...
    # 2) 嵌入两张图
    def embed(url):
        st, bd = _request("POST", "/embeddings", {
    ...
    st_e2, v2 = embed(url2)
    if not v1 or not v2:
        jprint("embed A", st_e1, str(v1)[:200]); jprint("embed B", st_e2, str(v2)[:200])

...
def run_a():
    ...
    for i in range(1, MAX_A + 1):
        ...
        ok = tags != ["无明显瑕疵"]
        print(
            f"[A try {i}] sim={r.get('similarity')} same={r.get('same_item')} "
...
def run_once(name, rp, pp, listing, sku, amt):
    ...
    print(
        f"[{name}] sim={r.get('similarity')} same={r.get('same_item')} "
...
out = {
    ...
    "B": run_once(
        "B",
        os.path.join(UP, "live_apple.png"),
        os.path.join(UP, "live_book.png"),
        "新鲜苹果·A级果",
        "APPLE-LIVE-B",
        39.0,
    ),
    "C": run_once(
        "C",
        os.path.join(UP, "live_tshirt.png"),
        os.path.join(UP, "live_tshirt_listing.png"),
        "纯棉T恤·品质承诺卡",
        "TSHIRT-LIVE-C",
        59.0,
    ),
...
with open(
    os.path.join(os.path.dirname(__file__), "live_demo_capture.json"), "w", encoding="utf-8"
) as f:

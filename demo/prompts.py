...
# ===================== ① 同款一致性比对（VL 直接判同款）=====================
# 百炼工作空间 OpenAI 兼容模式不支持视觉向量模型（tongyi-embedding-vision-plus），
# 故 ① 改用多模态模型「同时看两张图」直接判同款，比向量更贴合业务（同款/调包判定）。
SIMILARITY_PROMPT = (
    "你是一名跨境退货甄别员。下面第一张是「买家退回件」照片，第二张是「本店商品主图」。"
    "请判断这两张图是否为同一件商品（同款同物，允许轻微角度/光线差异；"
    "颜色、款式或明显外观不同则视为非同款）。只输出纯 JSON，不要任何解释、不要 markdown 围栏，"
    '格式为：{"similarity":0.0到1.0之间的小数,"same_item":true或false,"reason":"一句话依据"}。'
)


# ===================== ② 瑕疵视觉识别（多模态·双图对比）=====================
# 关键修复（P3-18）：退货法医的核心是「退回件 vs 本店主图」的差异取证，
# 单图孤立审瑕疵会把污渍/划痕误读为商品本身图案。改为同时喂两张图，让模型定位
# 退回件相对主图「新增」的瑕疵——与 ① 同款判定的双图输入保持一致口径。
DEFECT_RECOGNITION_PROMPT = (
    "你是一名严苛的跨境电商质检员。下面给你两张图："
    "**第一张是卖家上架的商品主图，第二张是买家申请退货的退回件实拍图**。"
    "请对比两图，只关注退回件相对主图**新增**的、与「买家退货原因」相关的视觉瑕疵，"
    "优先从以下类别中判定：外包装破损、缺件/少配件、污渍/水渍、使用痕迹、"
    "功能故障、颜色不符、材质不符、仿冒/假货、吊牌/标签缺失。"
    "用逗号分隔的简短中文标签列出你确认退回件新增的瑕疵（每个标签不超过 6 个字）。"
    "不要输出任何解释、序号或英文。若退回件相对主图无明显新增瑕疵，只输出：无明显瑕疵。"
)
...
DEFECT_BBOX_PROMPT = (
    "你是一名跨境电商质检员。下面两张图："
    "**第一张是卖家商品主图，第二张是买家退回件实拍图**。"
    "请对比两图，观察退回件相对主图**新增**的瑕疵所在位置，"
    "对每个你确认的新增瑕疵，输出一个边界框，使用归一化坐标（取值范围 0~1，"
    "图片左上角为原点，x、y 为框左上角坐标，w、h 为框的宽与高）。"
    "只从以下类别中选标签：外包装破损、缺件/少配件、污渍/水渍、使用痕迹、"
    "功能故障、颜色不符、材质不符、仿冒/假货、吊牌/标签缺失。"
    "严格只输出 JSON，不要任何解释或 markdown 围栏，格式为："
    '{"boxes":[{"label":"标签","bbox":[x,y,w,h],"confidence":0.9}]}。'
    '若退回件相对主图无明显新增瑕疵，只输出：{"boxes":[]}。'
)

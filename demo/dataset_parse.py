...
def _parse_thelook_csv(reader) -> list[dict]:
            ...
            out.append(
                {
                    ...
                    "category": cat,
                    "supplier": sup,
                    "supplier_name": SUPPLIERS.get(sup, sup),
                    "platform": "TheLook",
                    "language": "en",
                    "region": "US",
                    "amount": round(amt, 2),
                    ...
                    "same_item": True,
                    "defect_tags": defects,
                    "defect_description": "退货（无原因字段）",
                    "consistency": "待分析（无退货原因，无法判定责任归属）",
                    "outcome": "待分析",
                    "mode": "dataset",
                }
            )
    ...
    # TheLook 是数据集名而非平台：按品类×地区重映射到真实跨境电商平台
    # （remap_all=False：卖家自传的真实平台流水保持原样，仅修非平台值）
    apply_platform_mapping(out)
    return out
...
def _parse_amazon_csv(reader) -> list[dict]:
    ...
    for i, row in enumerate(reader, 1):
        try:
            ...
            sim, same, outcome, consistency = _amazon_derive(
                reason, seller_rating, return_score, defects
            )
            out.append(
                {
                    "case_id": cid,
                    ...
                    "region": "US",
                    "amount": amt,
                    "date": _to_date(row.get("order_datetime") or row.get("date")),
                    "similarity": sim,
                    "same_item": same,
                    "defect_tags": defects,
...
def _parse_rg_csv(reader) -> list[dict]:
    ...
    for i, row in enumerate(reader, 1):
        try:
            ...
            if not sku:
                continue
            cid = mapped.get("case_id") or (
                "RG-" + _stable_id(sku, mapped.get("date", ""), mapped.get("similarity", ""))
            )
            amt = _f(mapped.get("amount", 0))
            sim = _f(mapped.get("similarity", 0.9))
            same = (
                mapped.get("same_item", "").lower() in ("true", "1", "是", "yes")
            ) or sim >= SAME_ITEM_THRESHOLD
            defects = [
                t.strip()
                for t in re.split(r"[;,、]", mapped.get("defect_tags", "无明显瑕疵"))
                if t.strip()
            ] or ["无明显瑕疵"]
            out.append(
                {
                    ...
                    "platform": mapped.get("platform", "Amazon"),
                    "language": mapped.get("language", "en"),
                    "region": mapped.get("region", "US"),
                    "amount": round(amt, 2),
                    "date": _to_date(mapped.get("date")),
                    "similarity": round(sim, 3),
                    "same_item": same,
                    "defect_tags": defects,
                    "defect_description": mapped.get("defect_description", ""),
                    "consistency": mapped.get("consistency", ""),
                    "outcome": mapped.get("outcome", "待分析"),
...
def _parse_amazon_xlsx(rows, hdr) -> list[dict]:
    ...
    for r in rows[1:]:
        try:
            if str(
                r[idx.get("returned", -1)] if idx.get("returned", -1) >= 0 else ""
            ).strip() not in ("1", "1.0", "True", "true"):
                continue
            ...
            cat = str(
                r[idx.get("product_category", -1)] or ""
                if idx.get("product_category", -1) >= 0
                else ""
            )
            cid = "AMZ-" + pid
            ...
            out.append(
                {
                    ...
                    "amount": amt,
                    "date": _to_date(
                        r[idx.get("order_datetime", -1)]
                        if idx.get("order_datetime", -1) >= 0
                        else None
                    ),
                    "similarity": sim,
                    "same_item": same,
...
def _parse_uci_xlsx(rows, hdr) -> list[dict]:
    ...
    for r in rows[1:]:
        try:
            ...
            cid = "UCI-" + str(inv)
            desc = str(
                r[idx.get("description", -1)] or "" if idx.get("description", -1) >= 0 else ""
            )
            country = str(
                r[idx.get("country", -1)] or "GB" if idx.get("country", -1) >= 0 else "GB"
            )
            ...
            out.append(
                {
                    ...
                    "amount": amt,
                    "date": _to_date(
                        r[idx.get("invoicedate", -1)] if idx.get("invoicedate", -1) >= 0 else None
                    ),
                    "similarity": 0.85,
                    "same_item": True,
                    "defect_tags": defects,
    ...
    # 同上：UCI-Retail 是数据集名而非平台，重映射到真实跨境电商平台
    apply_platform_mapping(out)
    return out

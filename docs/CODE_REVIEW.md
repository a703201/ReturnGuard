...
> **当前版本**：2.1.1（仓库根 `VERSION` 为单一来源，与 `/api/config` 一致）
...
# 十一、大厂标准审查结论摘要（2026-08-29）

> 审查方式：结构 / 代码 / 安全 / 测试 / 文档 / 前端部署 六维实测打分（含公网实测与静态通读）。
> 基准版本：1.1.5（评分列为 **2026-08-29 当时快照**；其后的收口见 11.5）。

## 11.1 六维评分

| 维度 | 得分（当时） | 要点 | 现状 |
|---|---|---|---|
| 结构 | 6.0 | 分层清晰，但 `main.py` 仍偏巨型、部分职责可再拆 | ✅ 已按路由域拆分（`routers/*`，P1-9），`main.py` 仅留装配 |
| 代码 | 6.5 | 类型与 lint 门禁已绿；重复常量与长函数仍有优化空间 | ✅ 重复常量已下沉（`imghash` / `audio_utils` / `prompts`） |
| 安全 | 5.0 → **已修复** | P0 安全三洞 + SQLite WAL 失控，本轮全部闭环 | ✅ SEC-1 ~ SEC-13 全清零 |
| 测试 | 6.0 | 88 passed（当时），覆盖负向/鉴权/租户隔离/零覆盖链路 | ✅ **150 passed**（+ i18n / 静态资源版本化 / ROI 回测等回归） |
| 文档 | 5.5 | 内容完整但存在版本号/数字口径不一致（本轮已统一） | ✅ 已统一并加文档一致性守护测试 |
| 前端部署 | 5.5 | 单页零构建易部署；构建产物与资源治理偏弱 | ✅ 静态资源 URL 版本化 + `no-store`，缓存问题根治 |
| **综合** | **5.8 / 10** | 功能承诺落地度高，工程化与一致性是主要扣分项 | 主要扣分项已逐项收口 |

## 11.2 本轮已闭环项

- **P0 安全三洞**：已修复并回归验证。
- **SQLite WAL 失控**：已修复（WAL 文件增长与 checkpoint 策略收口）。
- **AI 诚实性标注**：已补齐——真实模型输出与 mock 回退在前端/接口层均显式区分标注（`capabilities` / `defect_boxes_live` / `mode`）。
- **CI 转绿**：流水线已恢复全绿。
...
| 项 | 说明 |
|---|---|
| 覆盖率门禁 | `--cov-fail-under=75` 已入 CI；继续补端到端 / 并发场景用例 |
| 大字段存储 | `dossier` / `voice_audio_b64` 仍为库内 Text，规模部署建议改对象存储 URL |

## 11.5 后续收口（2026-09-11 / 09-14）

| 收口 | 内容 |
|---|---|
| 四项闭环（09-11） | 母语多语 TTS · 正文级 i18n · ROI 回测 + A/B 台架 · 19 条 SQLAlchemy DeprecationWarning → 0 |
| 多语言（09-14） | 界面扩至 **zh / en / fr** 三语（各 355 键），数字/日期按 `locale` 格式化，补齐录入表单与登录弹窗漏抽文案 |
| 静态资源版本化（09-14） | `/static/*` 改 `no-store` + 全部模块 URL 带 `?v=<VERSION>`，根治「本机正常、公网升级不生效」（CDN 覆写 `no-cache` 为 `max-age=14400`） |
| 文档一致性（09-14） | 修正 SCHEMA.md 的 real 库名错误（→ `returnguard_real`）、字段数 27→28（补 `tenant_id`）、`schema.sql` 与 ORM 同步；API.md 补齐 8 个缺失端点与全部请求/响应字段；并新增文档漂移守护测试 |
| 端到端测试 | 当前以单元 + 集成为主，缺浏览器级 E2E 与并发压测 |
| GitCode 镜像 | 已创建并三路同步：`gitcode.com:a703201/ReturnGuard`（Public），与 GitHub `origin`、Gitea 内网镜像同步推送 |
| 演示视频 | 分镜脚本已就绪，mp4 尚未录制 |
